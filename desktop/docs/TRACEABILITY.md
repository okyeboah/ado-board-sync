...
| PRD-AC-07 desktop and CLI plans are identical | §3.3.3 | ABSD-302, ABSD-503 | `PlanParityTests` (18) — each command runs through the real CLI against its own `FakeClient` and through the port against `FakeBoardGateway`, and the **board each leaves behind** is compared field by field: dedup, close-children ×2, assign ×3, sprints, resync, resync-tasks, sync-one ×2 (a drifted Issue's update and a missing one's create), the audit verdict clean and drifted, and since 2026-09-19 import, `sync` (the whole chain), `set-state` and `advance` — advance probing one real git repository from both sides. `LiveBoardTests.TheResyncPlanNamesTheSameDescriptionsTheCliAuditNames` adds the live cross-check. `AcceptanceTests.TheDesktopPlanAndTheCliPlanAgree` fails if that gate shrinks | Covered — no divergence remains. The last one, `assign` writing to the highest id in the CLI and the lowest in the port on a board carrying a duplicated code, was closed on 2026-09-06: `dedup` keeps `min(ids)`, so the highest is the copy it deletes. `TheTwoImplementationsPickTheSameDuplicateForAssign` now asserts agreement |
...
| PRD-AC-10 no PAT blocks board actions | §3.1 | ABSD-103 | `PatResolverTests.*` — source order, whitespace handling, `DescribeSources` leaks no token; `AcceptanceTests.WithNoResolvableTokenEveryBoardActionIsBlockedAndTheSourcesAreNamed` asserts the board was never even read (`ReadCount` is 0, not merely "no writes") and that the status names the sources checked | Covered — including the three platform credential stores, which `CredentialStoreTests` covers through the injected process seam since 2026-09-05 |
...
| PRD-AC-17 released package runs without a toolchain | — | ABSD-601 | `AcceptanceTests.ThePackagingScriptsProduceASelfContainedBuildThatNeedsNoToolchain` pins what the criterion turns on — that `publish.sh` publishes `--self-contained` with `PublishSingleFile`, and that `package.sh` stamps unsigned output as such rather than letting it look installable; since 2026-09-19 the CI launch proof (`desktop/build/launch-proof.sh`, ABSD-506) starts that published binary on macOS, Windows and Linux runners and observes its window from outside the process — on macOS and Linux with the .NET SDK off `PATH`, on Windows with the dotnet directories stripped | Partial — the start-without-toolchain clause is CI-proven on all three OSes; what stays manual is the installer flow itself (installing the `.dmg`/`.zip`/`.tar.gz`, then launching what was installed), which no hosted runner exercises |
...
| ABSD-113 Decompose the shell view model | Three gates, one per deliverable: no `.cs` file in the solution exceeds 500 lines (checked by a line count in the final gate); `CompositionRootTests` direction tests stay green with the extractions in place; and the credential chain has exactly one home — `PatResolver.ForConfig` is called from `CredentialSession` alone, never from a view model. **Met** in the working tree; the row stays Partial until commit. |
...
| ABSD-506 CI to the desktop application | The workflow gates the desktop solution, and the packaging lanes end in `desktop/build/launch-proof.sh`: a real window observed from outside the process on macOS (CGWindowList), Linux (xdotool on Xvfb) and Windows (`MainWindowHandle`); the job fails when no window appears, so exit 0 alone is never evidence. **Met** on the run that carried it. |
| ABSD-307 `sync` as one Plan | `PlanBuilderStateTests` pin composition, documented order, the markup refusal and the quiet case; `PlanParityTests.SyncLeavesTheSameBoardTheCliLeaves` compares end boards with the CLI's `sync`. **Met.** |
| ABSD-308 `set-state` | `PlanBuilderStateTests` pin the state write, the `[ ]` tick, `--no-tick`, an explicit target state and unknown-id notes; `PlanParityTests.SetStateLeavesTheSameBoardTheCliLeaves` compares end boards. **Met.** |
| ABSD-309 `advance` | `PlanBuilderStateTests` pin evidence-to-advance, already-past unchanged, the never-Done rule, ghost notes and the unconfigured-states refusal; `PlanParityTests.AdvanceLeavesTheSameBoardTheCliLeaves` probes one real git repository from both sides. **Met.** |
...
| NFR-2 150ms preview re-render at 500 items | ABSD-203 | No perf test asserts it; the buffer recompute path is the same Core functions at fixture scale. `tests/test_performance.py` pins the CLI side | Open — the remaining work this row names is a benchmark, not behaviour |
...
| NFR-4 the PAT never reaches logs/exports/history/backlog | ABSD-103, ABSD-501 | `PatResolverTests.DescribeSourcesLeaksNoToken`; `DiagnosticRedaction` registers the resolved token so the JSONL sink cannot print it; `OperationsWiringTests` asserts no history or diagnostics event carries an item's title, and the history store's schema holds codes, operations, outcomes and timestamps only | Covered |
| NFR-5 CLI retry/backoff semantics in Apply | ABSD-301 | `LiveBoardTests` + `test_client.py` pin the CLI; the gateway ports the contract, create never retried | Partial — the retry rules remain ports until the write path runs live (see the High gap) |
...
| DS §6.1 keyboard-reachable editor and plan flow | ABSD-109 | `AccessibilityTests.CtrlSKeysSaveThroughTheKeyboardWithoutAMouse` sends the real keystroke through the headless input pipeline and asserts the file was written; `.TabMovesTheKeyboardFocusBetweenTheSurfaces` walks focus off the editor | Covered |
| DS §6.2 focus outlines in the accent | ABSD-109 | Focus styles for the restyled templates (buttons, nav items, tree items, editable editor) defined in `ControlStyles.axaml`; the two keyboard tests above land focus on them | Covered |
| DS §6.3/6.6 no colour-alone states | ABSD-109 | Unsaved chips and plan badges carry glyph + word by construction (`BacklogNodeViewModelTests` pins the summaries); the documented contrast pass lives in `AccessibilityTests` over Theme.axaml's own hex values, and DESIGN-SYSTEM §2 records its numbers | Covered |
...
| DS §6.5 screen-reader labels | ABSD-109 | Plan rows bind `AutomationProperties.Name` to "operation + code + title" as one phrase; the tree's `!` badge is named by its problem summary | Covered |
...
| NFR gates | 5 Covered, 2 Open, 2 Partial, 1 Covered-by-AC-rows, 1 Met-by-absence |
| DS §6 gates | 4 Covered, 1 met by absence |
