...
| ABSD-203 Split-pane editor with live preview | Done | The source pane is editable; the buffer recomputes preview, generated HTML, task list and markup problems per keystroke from the same Core functions. Save splices every dirty buffer back at the parser's own ranges, last-to-first, preserving EOL style, trailing newline and blank separators (`BacklogSplicerTests`, 11). Plan and Apply are refused while edits are unsaved. The one remainder this row used to carry — line-level gutter markers inside a description — is superseded, not outstanding: the converter escapes raw angle brackets on both implementations (the PRD-AC-03 decision of 2026-09-07), so no authored input can put a markup problem on a single line, and a per-line marker would be dead UI over a state the editor cannot reach. The problems card remains the per-item surface, and it is what blocks Apply. |
