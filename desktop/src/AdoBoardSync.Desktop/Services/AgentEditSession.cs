...
///     <list type="bullet">
///         <item>
///             The file is only ever left changed by an explicit accept. A run that
...
///             the diff would describe an accident.
///         </item>
///         <item>
///             Every run reaches <see cref="IAgentRunHistory" />, including the ones
///             nobody keeps. A run whose edit was thrown away is exactly the pattern the record
///             exists to make visible, and the verdict is written once, after the decision.
///         </item>
///     </list>
    ...
    IAgentRunHistory history)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (originalText.IsFailure) return originalText.Error!;
        ...
        if (run.IsFailure)
        {
            ...
            var noStart = await RecordAsync(
                request, startedAt, neverStarted, false,
        ...
        if (result.Status != AgentRunStatus.Succeeded)
        {
            ...
            var incomplete = await RecordAsync(request, startedAt, result, false, stopped)
                .ConfigureAwait(false);
        ...
        if (after.IsFailure)
        {
            var unreadable = await RecordAsync(
                request, startedAt, result, false,
        ...
        AgentEditProposal proposal, CancellationToken cancellationToken = default)
    {
        ...
        return VerdictAsync(proposal, true, cancellationToken);
    }
        ...
        AgentEditProposal proposal, CancellationToken cancellationToken = default)
    {
        ...
        if (Restore(review.Snapshot) is { } failure) return failure;

        return await VerdictAsync(proposal, false, cancellationToken).ConfigureAwait(false);
    }
