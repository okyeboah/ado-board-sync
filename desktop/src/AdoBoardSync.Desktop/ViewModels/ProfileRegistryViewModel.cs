...
public sealed partial class ProfileRowViewModel(ProfileEntry entry) : ObservableObject
{
    ...
    public string ProfileKey =>
        Core.Operations.ProfileKey.For(Entry.Org, Entry.Project);
}
...
public sealed partial class ProfileRegistryViewModel : ObservableObject
{
    ...
    [ObservableProperty] [NotifyPropertyChangedFor(nameof(HasActiveProfile))]
    ...
    /// <summary>
    ///     Raised after the active profile has changed, and never for a
    ///     re-selection of the profile already open.
    /// </summary>
    ...
    ///     The shell used to discard the returned task (<c>_ = Profiles?.AddAsync(...)</c>).
    ///     Everything that matters in <see cref="AddAsync" /> happens before its first
    ///     await — the entry is added, the registry is persisted, the collection is
    ///     republished — so an exception in any of it faults the returned task
    ///     synchronously, and the discard threw that away. A switcher that silently
    ///     never listed the open profile looked exactly like one that worked.
    /// </summary>
    public async Task AddSafelyAsync(BacklogWorkspace workspace, CancellationToken cancellationToken)
    {
        try
        {
            await AddAsync(workspace, cancellationToken).ConfigureAwait(true);
        }
        catch (Exception ex)
        {
            ErrorText =
    ...
    private async Task RaiseAsync(CancellationToken cancellationToken)
    {
        ...
        if (ProfileEntry.PathComparer.Equals(active?.ConfigPath ?? string.Empty, _announced ?? string.Empty)) return;

        _announced = active?.ConfigPath;

        if (ActiveProfileChanged is not { } subscribers) return;
