...
public static partial class PlanBuilder
{
        ...
        string backlogMarkdown)
    {
        ...
        foreach (var (key, id) in boardEpics) epicByTitle.TryAdd(key, id);
        ...
        foreach (var item in snapshot.Items.Where(i => i.WorkItemType == storyType).OrderBy(i => i.Id))
        {
            ...
            if (match.Success) existingIssues.TryAdd(match.Groups[1].Value.ToUpperInvariant(), item.Id);
        ...
        foreach (var item in items)
        {
            ...
            if (!planned.Add($"issue{code ?? title.ToLowerInvariant()}")) continue;
        ...
        string backlogMarkdown)
    {
        ...
        foreach (var item in items)
        {
            ...
            else if (item.Code is { Length: > 0 } code) wantIssues[code.ToUpperInvariant()] = (item.Title.Trim(), html);
        }
        ...
        foreach (var work in snapshot.Items.OrderBy(i => i.Id))
        {
            ...
            if (work.WorkItemType != storyType) continue;

            var match = config.IssueCodeRegex.Match(title);
            if (!match.Success) continue;

            var code = match.Groups[1].Value.ToUpperInvariant();
            if (!wantIssues.TryGetValue(code, out var want)) continue;

            var changes = new List<PlanFieldChange>();
            if (title != want.Title) changes.Add(new PlanFieldChange(BoardFieldChange.TitleField, title, want.Title));
        ...
        string backlogMarkdown)
    {
        ...
        foreach (var work in snapshot.Items)
        {
            ...
            if (match.Success) issueIdsByCode.TryAdd(match.Groups[1].Value.ToUpperInvariant(), work.Id);
        }
        ...
        {
            if (!issueIdsByCode.TryGetValue(code,
                    out var issueId)) continue; // No board Issue for this code — nothing to hang Tasks off.
            ...
            foreach (var (title, bullet) in desired)
                if (existing.All(e => e.Title != title))
                    rows.Add(new PlanRow
                    {
                        ...
                        // Named here rather than inferred from the command: inside
                        // a `sync` Plan the same rows run under PlanCommand.Sync,
                        // whose level-to-type mapping would create them as Issues.
