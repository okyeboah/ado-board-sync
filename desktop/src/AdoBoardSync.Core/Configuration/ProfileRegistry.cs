...
public sealed record ProfileEntry(string ConfigPath, string Org, string Project, string DisplayName)
{
    ...
    /// <summary>
    ///     What the switcher shows. Falls back to the board rather than to an
    ///     empty row: an unnamed profile is still identifiable by the board it opens.
    /// </summary>
    ...
    ///     paths and a caller may hold the relative one it was opened with, so the
    ///     comparison goes through the filesystem's idea of the path rather than the
    ...
    public static bool SamePath(string? left, string? right)
    {
        ...
        try
        {
            return PathComparer.Equals(Path.GetFullPath(left.Trim()), Path.GetFullPath(right.Trim()));
        }
...
public sealed record ProfileRegistry
{
    ...
    public ProfileEntry? Find(string? configPath)
    {
    ...
    public Result<ProfileRegistry> Add(ProfileEntry entry)
    {
        ...
        var existing = profiles.FindIndex(p => ProfileEntry.PathComparer.Equals(p.ConfigPath, normalised.ConfigPath));
