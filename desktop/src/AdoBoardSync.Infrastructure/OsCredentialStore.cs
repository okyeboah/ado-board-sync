...
public static class OsCredentialStore
{
    ...
    {
        if (OperatingSystem.IsMacOS()) return new MacOsKeychainCredentialStore();
...
internal static class CredentialProcess
{
    ...
    internal static Result<CredentialOutput> Run(string fileName, IReadOnlyList<string> arguments, string? stdin = null)
    {
        ...
        try
        {
            ...
            if (process is null) return Error.SourceFailure("credential.unavailable", $"Could not start {fileName}.");
...
public abstract class ProcessCredentialStore : ICredentialStore
{
    ...
    {
        if (!IsAvailable) return (string?)null;

        var run = _run(Tool, ReadArguments(key), null);
        if (run.IsFailure) return run.Error!;

        var output = run.Value;
        if (output.Succeeded) return output.Token;
    ...
    {
        if (!IsAvailable) return Error.SourceFailure("credential.unavailable", $"This machine has no {Name}.");
...
public sealed class MacOsKeychainCredentialStore : ProcessCredentialStore
{
    ...
    {
        return ["find-generic-password", "-s", key, "-w"];
    }
    ...
    {
        return ["add-generic-password", "-a", Environment.UserName, "-s", key, "-U", "-w"];
    }

    protected override IReadOnlyList<string> DeleteArguments(string key)
    {
        return ["delete-generic-password", "-s", key];
    }

    protected override string StandardInputFor(string secret)
    {
        return secret + "\n";
    }

    protected override bool IsMissing(int exitCode, string standardError)
    {
        return exitCode == ItemNotFound;
    }
}
...
public sealed class SecretToolCredentialStore : ProcessCredentialStore
{
    ...
    protected override IReadOnlyList<string> ReadArguments(string key)
    {
        return ["lookup", Attribute, key];
    }

    protected override IReadOnlyList<string> WriteArguments(string key)
    {
        return ["store", "--label", $"ADO Board Sync — {key}", Attribute, key];
    }

    protected override IReadOnlyList<string> DeleteArguments(string key)
    {
        return ["clear", Attribute, key];
    }
    ...
    protected override bool IsMissing(int exitCode, string standardError)
    {
        return standardError.Trim().Length == 0;
    }
