...
/// </summary>
public sealed partial class PlanParityTests : IDisposable
{
    ...
    private static IReadOnlyList<BoardWorkItem> CleanBoard(string epicHtml, string firstHtml, string secondHtml)
    {
        ...
        [
            new()
            {
                Id = 1, WorkItemType = "Epic", Title = "Epic 1: Foundation", Description = epicHtml, State = "Done"
            },
            new()
            {
                Id = 2, WorkItemType = "Issue", Title = "PROJ-101 · First issue", Description = firstHtml, ParentId = 1,
                State = "Active"
            },
            new()
            {
                Id = 3, WorkItemType = "Issue", Title = "PROJ-102 · Second issue", Description = secondHtml,
                ParentId = 1, State = "New"
            },
    ...
    {
        return ProfileWith(ConfigJson);
    }

    private (BoardConfig Config, IReadOnlyList<BacklogItem> Items, string Markdown) ProfileWith(
        string configJson)
    {
    ...
    private static string Html(params string[] lines)
    {
        return MarkdownHtml.ToHtml(lines);
    }
    ...
    public async Task DedupLeavesTheSameBoardTheCliLeaves()
    {
        ...
        var seed = new List<BoardWorkItem>(MatchingBoard())
        {
            new()
            {
                Id = 6, WorkItemType = "Issue", Title = "PROJ-101 · First issue (copy)", ParentId = 1, State = "New"
            },
    ...
    public async Task CloseChildrenWithAssignFromParentLeavesTheSameBoardTheCliLeaves()
    {
        ...
        var (cli, port) = await RunBothAsync(
            "close-children",
            (c, _, s, m) => PlanBuilder.BuildCloseChildren(c, s, m, true),
            seed,
            "--assign-from-parent");
    ...
    public async Task AssignWithoutTasksLeavesTheSameBoardTheCliLeaves()
    {
        ...
        var (cli, port) = await RunBothAsync(
            "assign",
            (c, _, s, m) => PlanBuilder.BuildAssign(c, s, m, false),
            seed,
            "--no-tasks");
    ...
    public async Task SprintsAssignOnlyLeavesTheSameBoardTheCliLeaves()
    {
        ...
        var (cli, port) = await RunBothAsync(
            "sprints",
            (c, _, s, m) => PlanBuilder.BuildSprints(c, s, m, true),
            seed,
            "--assign-only");
    ...
    public async Task TheTwoImplementationsPickTheSameDuplicateForAssign()
    {
        ...
        var seed = new List<BoardWorkItem>(MatchingBoard())
        {
            new()
            {
                Id = 9, WorkItemType = "Issue", Title = "PROJ-101 · First issue (copy)", ParentId = 1, State = "New"
            }
        };
        ...
        Assert.Contains(cli,
            line => line.StartsWith("#2 ", StringComparison.Ordinal) && line.Contains("assigned=ada@example.com"));
        Assert.DoesNotContain(cli,
            line => line.StartsWith("#9 ", StringComparison.Ordinal) && line.Contains("assigned=ada@example.com"));
    }
}